...
public class AISolverButton extends JButton {
	...
	private class AISolverActionListener implements ActionListener {
		...
		// Brute-force approach: Toggle lights that are on in a sequence until the
		// puzzle is solved
		// Gauss-Jordan Elimination was attempted but could not be completed before the
		// given due date
		private int solveWithAI(LightsOutGrid lightsOutGrid) {
			int moves = 0;

			while (!winChecker.checkWin(lightsOutGrid.getLights())) {
