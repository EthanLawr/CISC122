...
public class GemStacker extends JFrame implements ActionListener
{

	public static void main(String[] args) 
	{
		GemStacker light = new GemStacker();		//Creates a new LightsOut object, which is a new game.
	...
	public GemStacker()
	{
		...
		//Buttons
		JPanel buttonPanel = new JPanel();				// Panel for "puzzle"
		gameButtons = new JButton[5][5];				// 2D array to hold the board values
		buttonPanel.setLayout(new GridLayout(5,5));		// Grid for board				
		
		for(int i = 0; i<5; i++)						//these two loops will add each button into the two dimensional array.
		{
			for(int j = 0; j<5; j++)
			{
				int random = (int)(Math.random()*3);	// Decides whether or not a starting object will be a gem
		...
		controls.add(options, "South"); //adds the randomize button to the controls JPanel.
        ...
        highScoreLabel.setLayout(new GridLayout(1,1));			//sets the layout of the labels JPanel.
        highScoreLabel.add(highscore = new JLabel());					//adds the wins label to the label JPanel.
		highscore.setText("Name    | Moves" + winCount);
		mainPanel.add(highScoreLabel, "East");
